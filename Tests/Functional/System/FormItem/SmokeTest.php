...
namespace TMSolution\DynamicFormBundle\Tests\Functional\Model\System\FormItem;
...
class SmokeTest extends CommandTestCase {
    ...
    protected static $entityClass='TMSolution\DynamicFormBundle\Entity\FormItem';
    ...
    public function testCreate()
    {
        ...
        $form = $crawler->selectButton('create')->form([
                'system_form-item_insert[_token]'  => $this->getToken($crawler,'system_form-item_insert[_token]'),                
                'system_form-item_insert[name]'  => $entity->getName(),                
                'system_form-item_insert[formType]'  => $entity->getFormType(),                
                'system_form-item_insert[formPage]'  => $entity->getFormPage(),                
                'system_form-item_insert[position]'  => $entity->getPosition(),                
                'system_form-item_insert[required]'  => $entity->getRequired(),    
        ]);
    ...
    public function testUpdate()
    {
        ...
        $form = $crawler->selectButton('update')->form([
                'system_form-item_edit[_token]'  => $this->getToken($crawler,'system_form-item_edit[_token]'),                
                'system_form-item_edit[name]'  => $entity->getName(),                
                'system_form-item_edit[formType]'  => $entity->getFormType(),                
                'system_form-item_edit[formPage]'  => $entity->getFormPage(),                
                'system_form-item_edit[position]'  => $entity->getPosition(),                
                'system_form-item_edit[required]'  => $entity->getRequired(),    
        ]);
    ...
    {
        $route='/system/form-item/delete/1';
        $crawler = self::$client->request('GET', $route);
        $this->printError($crawler);
        $this->assertEquals(200, self::$client->getResponse()->getStatusCode(), sprintf("Unexpected HTTP status code for GET %s",$route)); 
    ...
    public function testDelete()
    {
        ...
        $form = $crawler->selectButton('delete')->form(array(
            'system_form-item_delete[_token]'  => $this->getToken($crawler,'system_form-item_delete[_token]'),
        ));
    ...
    public function testFilter()
    {
        ...
        [
            'system_form-item_filter' =>[                
                'name'  => $entity->getName(),                
                'formType'  => $entity->getFormType(),                
                'formPage'  => $entity->getFormPage(),                
                'formItemOptionsValues'  => $entity->getFormItemOptionsValues(),                
    ...
    {
        $route='/system/form-item/typeahead/name';
        $entity=$this->getEntity();
        $crawler = self::$client->request('GET', $route,[
            'system_form-item_filter' =>[                
                'name'  => $entity->getName(),                
