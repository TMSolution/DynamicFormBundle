...
namespace TMSolution\DynamicFormBundle\Tests\Functional\Model\System\DataType;
...
class SmokeTest extends CommandTestCase {
    ...
    protected static $entityClass='TMSolution\DynamicFormBundle\Entity\DataType';
    ...
    public function testCreate()
    {
        ...
        $form = $crawler->selectButton('create')->form([
                'system_data-type_insert[_token]'  => $this->getToken($crawler,'system_data-type_insert[_token]'),                
                'system_data-type_insert[name]'  => $entity->getName(),                
                'system_data-type_insert[technicalName]'  => $entity->getTechnicalName(),                
                'system_data-type_insert[formTypeOptions]'  => $entity->getFormTypeOptions(),    
        ]);
    ...
    public function testUpdate()
    {
        ...
        $form = $crawler->selectButton('update')->form([
                'system_data-type_edit[_token]'  => $this->getToken($crawler,'system_data-type_edit[_token]'),                
                'system_data-type_edit[name]'  => $entity->getName(),                
                'system_data-type_edit[technicalName]'  => $entity->getTechnicalName(),                
                'system_data-type_edit[formTypeOptions]'  => $entity->getFormTypeOptions(),    
        ]);
    ...
    {
        $route='/system/data-type/delete/1';
        $crawler = self::$client->request('GET', $route);
        $this->printError($crawler);
        $this->assertEquals(200, self::$client->getResponse()->getStatusCode(), sprintf("Unexpected HTTP status code for GET %s",$route)); 
    ...
    public function testDelete()
    {
        ...
        $form = $crawler->selectButton('delete')->form(array(
            'system_data-type_delete[_token]'  => $this->getToken($crawler,'system_data-type_delete[_token]'),
        ));
    ...
    {
        $route='/system/data-type/typeahead/name';
        $entity=$this->getEntity();
        $crawler = self::$client->request('GET', $route,[
            'system_data-type_filter' =>[                
                'name'  => $entity->getName(),                
